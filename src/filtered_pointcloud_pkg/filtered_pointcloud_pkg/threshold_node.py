...
class PCDFilter(Node):
    def __init__(self):
        ...
        self.cluster_location = self.create_publisher(Point, 'cluster_point', 10)
    ...
    latest_points: PointCloud2 | None = None
    ...
    def filter_callback(self, pointcloud_msg):
        ...
        if largest_cluster_value == 999:
            ...
            # Publishing to our publisher our filtered pointcloud and the distance of that 
            self.publisher_.publish(filtered_msg)
            self.publish_cluster_distance.publish(distance_msg)
